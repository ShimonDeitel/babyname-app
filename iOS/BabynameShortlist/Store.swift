...
final class Store: ObservableObject {
    @Published var entries: [BabynameShortlistEntry] = []
    ...
    init() {
        ...
        let dir = appSupport.appendingPathComponent("BabynameShortlist", isDirectory: true)
    ...
    func add(_ entry: BabynameShortlistEntry) -> Bool {
    ...
    func update(_ entry: BabynameShortlistEntry) {
    ...
    func delete(_ entry: BabynameShortlistEntry) {
    ...
    private func load() {
        if let data = try? Data(contentsOf: fileURL),
           let decoded = try? JSONDecoder().decode([BabynameShortlistEntry].self, from: data) {
            entries = decoded
        } else {
