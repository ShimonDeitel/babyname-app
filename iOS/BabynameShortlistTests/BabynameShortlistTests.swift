import XCTest
@testable import BabynameShortlist

@MainActor
final class BabynameShortlistTests: XCTestCase {
    ...
    func test_add_incrementsCount() {
        ...
        _ = store.add(BabynameShortlistEntry())
    ...
    func test_add_respectsFreeLimit() {
        ...
        let result = store.add(BabynameShortlistEntry())
    ...
    func test_update_modifiesExistingEntry() {
        let store = Store()
        var entry = BabynameShortlistEntry()
        _ = store.add(entry)
